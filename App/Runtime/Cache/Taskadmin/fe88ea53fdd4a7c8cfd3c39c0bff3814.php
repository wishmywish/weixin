...
html, body {
    ...
    <body>
        ...
        <select id="shopClassSelect">
            <?php if(is_array($tree)): $k = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><option  value="<?php if ($reModi['list']['f_id'] == $vo['f_id']) { echo $vo['f_classpid'];}else{echo $vo['f_id'];} ?>" <?php if ($reModi['list']['f_id'] == $vo['f_id']) { echo 'selected';} ?>  >
        ...
        <span>分类名称：</span></span> <input type="text" id="title" name="title" placeholder="请输入分类名称" value="<?php echo ($reModi["list"]["f_classname"]); ?>"><br><br>

        <input type="hidden" id="classid" name="classid" value="<?php echo ($reModi["list"]["f_id"]); ?>">
